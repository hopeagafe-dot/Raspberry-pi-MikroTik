...
def load_state() -> Dict[str, Any]:
    ...
    for entry in raw.get("users", []):
        ...
        users_dict[name] = {
            ...
            "limit_gb_original": float(entry.get("limit_gb_original") or entry.get("limit_gb", 100.0) or 100.0),
...
def save_state(state: Dict[str, Any]) -> None:
    ...
    for name in sorted(state["users"].keys()):
        ...
        out_users.append({
            ...
            "limit_gb_original": float(info.get("limit_gb_original") or info.get("limit_gb", 100.0) or 100.0),
...
def archive_daily_data(current_usage_state, target_date):
    try:
        ...
        # 저장 시점의 overhead_display_pct를 함께 기록 (표시 보정값 이력 보존)
        overhead_pct = 0.0
        try:
            bd = load_billing_date()
            overhead_pct = float(bd.get("overhead_display_pct", 0.0) or 0.0)
        except Exception:
            pass
...
def main():
    ...
    # RouterOS에서 comment 읽어 state 갱신
    # ★ limit_gb는 RouterOS에서 읽지 않음 — overhead 역산값이 저장되어 있으므로
    #   limit_gb_original(화면 표시 한도)은 manager.html에서만 변경 가능
    hotspot_user_info = get_hotspot_users_full(api)
    for name, info in hotspot_user_info.items():
        entry = users_state.setdefault(name, {
            "comment": "",
            "limit_gb": 100.0,
            "limit_gb_original": 100.0,
            "total_bytes": 0,
            "_last_raw_bytes": 0,
            "_offset_bytes": 0,
        })
        entry["comment"] = info["comment"]
        # limit_gb_original 최초 초기화 (아직 없는 경우에만)
        if "limit_gb_original" not in entry or not entry.get("limit_gb_original"):
            entry["limit_gb_original"] = info["limit_gb"]
            entry["limit_gb"] = info["limit_gb"]
        # limit_gb는 limit_gb_original과 항상 동기화 (화면 표시용)
        entry["limit_gb"] = entry["limit_gb_original"]
    ...
    for user in router_usernames:
        ...
        entry = users_state.setdefault(user, {
            "comment": hotspot_user_info.get(user, {}).get("comment", ""),
            "limit_gb": hotspot_user_info.get(user, {}).get("limit_gb", 100.0),
            "limit_gb_original": hotspot_user_info.get(user, {}).get("limit_gb", 100.0),
            "total_bytes": 0,
            "_last_raw_bytes": 0,
            "_offset_bytes": 0,
        })
        # limit_gb_original 이 아직 없는 기존 항목 보완 (한 번만 실행)
        if "limit_gb_original" not in entry or not entry["limit_gb_original"]:
            entry["limit_gb_original"] = entry.get("limit_gb", 100.0)

        total = int(entry.get("total_bytes", 0) or 0)
        raw_last = int(entry.get("_last_raw_bytes", 0) or 0)
        diff = total - raw_now
        ...
        entry["total_bytes"] = total_new
        entry["_last_raw_bytes"] = raw_now
