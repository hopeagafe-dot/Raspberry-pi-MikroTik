...
def _set_routeros_user_limit(username: str, limit_bytes: int) -> tuple:
    """RouterOS SSH: 특정 사용자의 limit-bytes-total 변경."""
    # limit_bytes=0 이면 무제한 (RouterOS 기본값)
    cmd = f'/ip hotspot user set "{username}" limit-bytes-total={int(limit_bytes)}'
...
def _apply_overhead_to_all_limits(overhead_pct: float) -> dict:
    """
    usage_public.json의 모든 사용자에 대해
    RouterOS limit = limit_gb_original / (1 + overhead_pct/100) 으로 SSH SET.
    """
    raw = _safe_read_json(USAGE_PUBLIC_PATH, {"users": []})
    ok_list, fail_list = [], []
    for u in raw.get("users", []):
        name = u.get("user")
        if not name:
            continue
        original_gb = float(u.get("limit_gb_original") or u.get("limit_gb") or 0.0)
        if original_gb <= 0:
            continue
        raw_limit_bytes = int(original_gb / (1 + overhead_pct / 100) * 1_000_000_000)
        ok, msg = _set_routeros_user_limit(name, raw_limit_bytes)
        if ok:
            ok_list.append(name)
        else:
            fail_list.append({"user": name, "error": msg})
            app.logger.error(f"[apply_overhead_limits] {name}: {msg}")
...
def load_usage_public():
    ...
    for u in raw.get("users", []):
        ...
        by_name[name] = {
            ...
            "limit_gb_original": float(u.get("limit_gb_original") or u.get("limit_gb", 100.0) or 100.0),
...
BILLING_ENGINE_PARAMS_DEFAULT = {
    "_comment": "요금 계산 엔진 상수 설정 파일. slope 기반 정산: Total$=E1+slope×usage, slope=(F16-F21+nMgr×mgDisc+nAp×apDisc-N×E1)/total_usage",
    "billing_inputs": {
        "base_amount_E1": 20.0,
        "target_total_F16": 1750.0,
        "common_prepaid_F21": 50.0
    },
    "manager":    {"discount": 30.0},
    "apprentice": {"discount": 30.0}
}
...
def api_users():
    ...
    for name, info in sorted(users_by_name.items()):
        ...
        result_users.append({
            ...
            "role": role,
            "limit_gb": limit_gb,
            "limit_gb_original": float(info.get("limit_gb_original") or limit_gb),
    ...
    # overhead_display_pct: billing_date.json에서 읽기
    try:
        _bd = _load_billing_date_raw()
        overhead_display_pct = float(_bd.get("overhead_display_pct", 0.0) or 0.0)
    except Exception:
        overhead_display_pct = 0.0

    return jsonify({
        "period_start_utc": period_start_utc,
        "last_updated_utc": usage_public.get("last_updated_utc"),
        "overhead_display_pct": overhead_display_pct,
        "users": result_users,
    })
...
def api_monthly_usage():
    ...
    # overhead_display_pct: billing_date.json에서 읽기
    try:
        _bd2 = _load_billing_date_raw()
        overhead_display_pct = float(_bd2.get("overhead_display_pct", 0.0) or 0.0)
    except Exception:
        overhead_display_pct = 0.0
...
def _read_daily_effective_bytes_for_user(path: Path, username: str):
    """archive 파일에서 (effective_bytes, overhead_pct) 반환. 없으면 None."""
    try:
    ...
    if not isinstance(users, list):
        return None
    # archive 파일에 저장된 overhead_pct (없으면 0)
    overhead_pct = float(raw.get("overhead_pct", 0.0) or 0.0)
    for u in users:
        if not isinstance(u, dict):
            continue
...
def api_daily_usage():
    ...
    # ★ 안전한 baseline 전략: 시작일 2일 전부터 로드
    # (클라이언트 시간대와 서버 UTC 차이로 인한 날짜 불일치 방지)
    # 1일 전 파일 우선 사용, 없으면 2일 전 파일 사용
    baseline1_date = d0 - timedelta(days=1)
    baseline2_date = d0 - timedelta(days=2)

    baseline1_path = DAILY_ARCHIVES_DIR / f"usage_daily_{baseline1_date.strftime('%Y-%m-%d')}.json"
    baseline2_path = DAILY_ARCHIVES_DIR / f"usage_daily_{baseline2_date.strftime('%Y-%m-%d')}.json"

    baseline1_res = _read_daily_effective_bytes_for_user(baseline1_path, username) \
                    if baseline1_path.exists() else None
    baseline2_res = _read_daily_effective_bytes_for_user(baseline2_path, username) \
                    if baseline2_path.exists() else None

    # tuple(effective_bytes, overhead_pct) 또는 None
    baseline1_eff = baseline1_res[0] if baseline1_res is not None else None
    baseline2_eff = baseline2_res[0] if baseline2_res is not None else None

    # 더 최근의 유효한 baseline 선택 (1일 전 우선)
    baseline_eff = baseline1_eff if baseline1_eff is not None else baseline2_eff
    baseline_used = baseline1_date if baseline1_eff is not None else \
                    (baseline2_date if baseline2_eff is not None else None)

    # 오늘 live 용 overhead_pct (billing_date.json 현재값)
    try:
        _bd_live = _load_billing_date_raw()
        live_overhead_pct = float(_bd_live.get("overhead_display_pct", 0.0) or 0.0)
    except Exception:
        live_overhead_pct = 0.0

    series = []
    # prev_eff 초기값: baseline 파일이 있으면 그 값, 없으면 None
    prev_eff = baseline_eff
    cum = 0
    for d in _daterange(d0, d1):
        ymd = d.strftime("%Y-%m-%d")
        path = DAILY_ARCHIVES_DIR / f"usage_daily_{ymd}.json"
        res = _read_daily_effective_bytes_for_user(path, username) if path.exists() else None
        eff = res[0] if res is not None else None
        overhead_pct = res[1] if res is not None else live_overhead_pct
        if eff is None:
            # 해당 날짜 파일 없음 → missing 처리 (prev_eff 갱신 안 함)
            series.append({"date": ymd, "effective_bytes": None, "daily_bytes": 0, "cumulative_bytes": cum, "overhead_pct": live_overhead_pct, "missing": True})
            continue
        if prev_eff is None:
            daily = eff if eff > 0 else 0
        else:
            daily = max(0, eff - prev_eff) if eff >= prev_eff else eff
        cum += daily
        prev_eff = eff
        series.append({"date": ymd, "effective_bytes": eff, "daily_bytes": daily, "cumulative_bytes": cum, "overhead_pct": overhead_pct, "missing": False})
    ...
    return _add_cors(jsonify({
        ...
        "start": start_s,
        "end": end_s,
        "baseline_date": baseline_used.strftime("%Y-%m-%d") if baseline_used else None,
        "baseline_found": baseline_eff is not None,
        "series": series
    })), 200
...
def api_users_save():
    ...
    # superadmin / admin 모두: limit_gb_original 갱신 + RouterOS SET
    # (overhead 역산해서 RouterOS raw limit 계산)
    try:
        _bd = _load_billing_date_raw()
        overhead_pct = float(_bd.get("overhead_display_pct", 0.0) or 0.0)
    except Exception:
        overhead_pct = 0.0

    raw_usage_for_limit = _safe_read_json(
        USAGE_PUBLIC_PATH,
        {"last_updated_utc": None, "users": []},
    )
    users_list_for_limit = raw_usage_for_limit.get("users", [])
    user_map_for_limit = {e.get("user"): e for e in users_list_for_limit if e.get("user")}
    limit_changed = False

    for u in users_in:
        name = u.get("user")
        if not name or "limit_gb_original" not in u:
            continue
        try:
            new_orig = float(u["limit_gb_original"])
        except Exception:
            continue
        if new_orig <= 0:
            continue
        entry = user_map_for_limit.get(name)
        if entry is None:
            continue
        old_orig = float(entry.get("limit_gb_original") or entry.get("limit_gb") or 0.0)
        if abs(new_orig - old_orig) < 0.001:
            continue  # 변화 없음
        entry["limit_gb_original"] = new_orig
        entry["limit_gb"] = new_orig
        # RouterOS raw limit = original / (1 + overhead%/100)
        raw_bytes = int(new_orig / (1 + overhead_pct / 100) * 1_000_000_000)
        ok, msg = _set_routeros_user_limit(name, raw_bytes)
        if not ok:
            app.logger.error(f"[users/save limit] RouterOS SET failed for {name}: {msg}")
        limit_changed = True

    if limit_changed:
        raw_usage_for_limit["last_updated_utc"] = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
        _safe_write_json(USAGE_PUBLIC_PATH, raw_usage_for_limit)
...
# ===== /api/settings/overhead_display_pct =====
@app.route("/api/settings/overhead_display_pct", methods=["GET", "POST"])
def api_overhead_display_pct():
    if not require_login():
        return jsonify({"error": "unauthorized"}), 401
    ...
    if request.method == "GET":
        try:
            bd = _load_billing_date_raw()
            pct = float(bd.get("overhead_display_pct", 0.0) or 0.0)
            return jsonify({"overhead_display_pct": pct})
        except Exception as e:
            return jsonify({"error": f"billing_date.json read failed: {e}"}), 500

    data = request.get_json(silent=True) or {}
    pct_val = data.get("overhead_display_pct")
    try:
        pct_val = float(pct_val)
    except (TypeError, ValueError):
        return jsonify({"error": "overhead_display_pct must be a number (0 ~ 20)"}), 400
    if not (0.0 <= pct_val <= 20.0):
        return jsonify({"error": "overhead_display_pct out of range (0 ~ 20)"}), 400

    try:
        bd = _load_billing_date_raw()
        bd["overhead_display_pct"] = round(pct_val, 2)
        _safe_write_json(BILLING_DATE_PATH, bd)
        # 전 사용자 RouterOS limit 재계산 및 SET
        limit_results = _apply_overhead_to_all_limits(pct_val)
        return jsonify({
            "status": "ok",
            "overhead_display_pct": bd["overhead_display_pct"],
            "limits_applied": limit_results,
        })
    except Exception as e:
...
def api_billing_engine_params():
    ...
    # billing_inputs(E1/F16/F21): admin/superadmin 모두 가능
    # manager.discount / apprentice.discount: admin/superadmin 모두 가능
    ...
    try:
        ...
        if "base_amount_E1"   in bi: _check_nonneg(bi["base_amount_E1"],   "billing_inputs.base_amount_E1")
        if "target_total_F16" in bi: _check_nonneg(bi["target_total_F16"], "billing_inputs.target_total_F16")
        if "common_prepaid_F21" in bi: _check_nonneg(bi["common_prepaid_F21"], "billing_inputs.common_prepaid_F21")
        mg = data.get("manager", {})
        if "discount" in mg: _check_nonneg(mg["discount"], "manager.discount")
        ap = data.get("apprentice", {})
        if "discount" in ap: _check_nonneg(ap["discount"], "apprentice.discount")
    except ValueError as e:
        return jsonify({"error": str(e)}), 400

    params = load_billing_engine_params()
    for section in ("billing_inputs", "manager", "apprentice"):
...
# ===== 요금 정산 엔진 (slope 기반, 오차 $0 보장) =====
def _run_billing_engine(usage_source, users_by_name, E1, F16, F21, billing_period_start_utc, engine_params=None):
    """
    신규 slope 기반 정산 엔진.
    Total $ = E1 + slope × usage_gb
    slope = (adj_target - N×E1) / total_usage
    adj_target = F16 - F21 + nMgr×mgDisc + nAp×apDisc
    sum(final_fee) + F21 = F16  (오차 없음)
    """
    usage_map = {}
    for u in usage_source.get("users", []):
        ...
        if "total_bytes" in u:
            try:
                ...
                off = int(u.get("_offset_bytes", 0) or 0)
                gb  = max(0, b + off) / (1000 ** 3)
            except Exception:
                gb = 0.0
        else:
            try:
                gb = float(u.get("total_gb", u.get("usage_gb", 0.0)) or 0.0)
            except Exception:
                gb = 0.0
        usage_map[name] = gb
    ...
    if not raw_users:
        return {
            ...
            "users":   [],
            "params":  {"base_amount_E1": E1, "target_total_F16": F16, "common_prepaid_F21": F21},
        }

    # 엔진 파라미터
    _ep     = engine_params or {}
    mg_disc = float(_ep.get("manager",    {}).get("discount", 30))
    ap_disc = float(_ep.get("apprentice", {}).get("discount", 30))

    N           = len(raw_users)
    n_mgr       = sum(1 for u in raw_users if u["role"] == "manager")
    n_ap        = sum(1 for u in raw_users if u["role"] == "apprentice")
    total_usage = sum(u["usage_gb"] for u in raw_users)

    # adj_target: 할인액 보정 후 목표 수금액
    adj_target = F16 - F21 + n_mgr * mg_disc + n_ap * ap_disc

    # slope ($/GB)
    slope = (adj_target - N * E1) / total_usage if total_usage > 0 else 0.0

    sum_final = 0.0
    for u in raw_users:
        pure_fee  = E1 + slope * u["usage_gb"]
        disc      = mg_disc if u["role"] == "manager" else (ap_disc if u["role"] == "apprentice" else 0.0)
        u["base_fee"]     = float(round(E1, 2))
        u["variable_fee"] = float(round(slope * u["usage_gb"], 2))
        u["discount"]     = float(disc)
        u["final_fee"]    = float(round(max(0, pure_fee - disc), 2))
        sum_final        += u["final_fee"]

    check_total = round(sum_final + F21, 2)

    summary = {
        "E1":                    round(E1, 2),
        "slope":                 round(slope, 4),
        "usage_sum_gb":          round(total_usage, 3),
        "sum_final":             round(sum_final, 2),
        "check_total_collected": check_total,
        "target_total":          round(F16, 2),
    }
...
def api_snapshot_save():
    ...
    try:
        ...
        period_start_utc = _calc_period_start_from_billing_date()
        # 저장 시점의 overhead_display_pct 함께 기록
        try:
            _bd_snap = _load_billing_date_raw()
            snap_overhead_pct = float(_bd_snap.get("overhead_display_pct", 0.0) or 0.0)
        except Exception:
            snap_overhead_pct = 0.0
        snapshot_data = {
